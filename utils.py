...
async def create_agent_instance(docker: Docker,
                                agent_id: str,
                                api_url: str,
                                sk_value: str
                                ):
...
async def delete_agent_instance(
    ...
    destination = pathlib.Path(f'./data/{agent_id}.DELETED.{int(time.time())+config.EXPIRY}')
    try:
        # 设置文件删除标志
